...
tag @e[scores={VisTestCount=4,FightState=1},tag=AskedVisTest] add BlockVisTests

# Switches to the Waiting Fight State
scoreboard players set @e[scores={VisTestCount=6..,FightState=1},tag=AskedVisTest] NewFightState 2

# If it's a main AI, goes on the other side of the player to try to find targets
#scoreboard players set @e[scores={VisTestCount=6..,FightState=1},tag=AskedVisTest,tag=MainAI] NewFightState 4
