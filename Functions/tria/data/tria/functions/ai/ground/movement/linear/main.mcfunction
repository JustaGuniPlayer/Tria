...
execute as @e[tag=LinearMoving] positioned ^ ^ ^1 unless block ~ ~ ~ #tria:cangothrough run scoreboard players set @s Var2 1

execute as @e[tag=LinearMoving, scores={Var2=0}] positioned ^ ^ ^1 if block ~ ~-1 ~ #tria:cangothrough run scoreboard players set @s Var2 -1
execute as @e[tag=LinearMoving, scores={Var2=-1}] positioned ^ ^ ^1 if block ~ ~-2 ~ #tria:cangothrough run tag @s add noMove
